// 音声ミックス + 最終MP4作成（エスタファ版・抑制トーン）
// 演出: 42.3-43.32秒はBGM無音（赤フラッシュ→静寂→唯一の低音ドン）/ 52.8秒からフェードアウト
...
let fc = '[1:a]loudnorm=I=-15:TP=-1.5:LRA=11,aresample=48000[voice];'
  + `[8:a]atrim=0:${TL.DUR},volume=-17.1dB,aresample=48000,`
  + `volume='if(between(t,42.3,43.32),0,1)':eval=frame,`
  + 'afade=t=in:st=0:d=1.2,afade=t=out:st=52.8:d=1.2[bgm];';
...
execFileSync('ffmpeg', [
  ...
  '-movflags', '+faststart', '-t', String(TL.DUR), O('philippines_estafa_final.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_estafa_final.mp4');

execFileSync('ffmpeg', [
  '-y', '-v', 'error', '-i', O('philippines_estafa_final.mp4'),
  '-c:v', 'libx264', '-preset', 'slow', '-crf', '21',
  '-maxrate', '3500k', '-bufsize', '7000k', '-pix_fmt', 'yuv420p',
  '-c:a', 'copy', '-movflags', '+faststart', O('philippines_estafa_tiktok.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_estafa_tiktok.mp4');

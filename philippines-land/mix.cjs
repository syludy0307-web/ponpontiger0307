// 音声ミックス + 最終MP4作成（名義貸し土地版）
// 演出: カット④(12.15-13.18)はBGM無音 / 57.3秒からBGMフェードアウト → ラスト無音＋赤画面
...
let fc = '[1:a]loudnorm=I=-15:TP=-1.5:LRA=11,aresample=48000[voice];'
  ...
  + `volume='if(between(t,12.15,13.18),0,1)':eval=frame,`
  + 'afade=t=in:st=0:d=0.4,afade=t=out:st=57.3:d=1.2[bgm];';
...
execFileSync('ffmpeg', [
  ...
  '-movflags', '+faststart', '-t', String(TL.DUR), O('philippines_land_final.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_land_final.mp4');

execFileSync('ffmpeg', [
  '-y', '-v', 'error', '-i', O('philippines_land_final.mp4'),
  '-c:v', 'libx264', '-preset', 'slow', '-crf', '21',
  '-maxrate', '3500k', '-bufsize', '7000k', '-pix_fmt', 'yuv420p',
  '-c:a', 'copy', '-movflags', '+faststart', O('philippines_land_tiktok.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_land_tiktok.mp4');

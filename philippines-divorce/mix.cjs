// 音声ミックス + 最終MP4作成（離婚制度なし版）
// narration -15 LUFS + BGM(-28 LUFSベッド) + SFX 23発 → 高品質版と配信用(30MiB以下)を出力
...
execFileSync('ffmpeg', [
  ...
  '-movflags', '+faststart', '-t', String(TL.DUR), O('philippines_divorce_final.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_divorce_final.mp4');

// 配信用（30MiB以下にビットレート上限）
execFileSync('ffmpeg', [
  '-y', '-v', 'error', '-i', O('philippines_divorce_final.mp4'),
  '-c:v', 'libx264', '-preset', 'slow', '-crf', '21',
  '-maxrate', '3600k', '-bufsize', '7200k', '-pix_fmt', 'yuv420p',
  '-c:a', 'copy', '-movflags', '+faststart', O('philippines_divorce_tiktok.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_divorce_tiktok.mp4');

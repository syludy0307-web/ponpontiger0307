// 音声ミックス + 最終MP4作成（RA9262版）
// narration -15 LUFS + BGM(-28 LUFSベッド) + SFX 25発 → 高品質版と配信用(30MiB以下)を出力
...
execFileSync('ffmpeg', [
  ...
  '-movflags', '+faststart', '-t', String(TL.DUR), O('philippines_ra9262_final.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_ra9262_final.mp4');

// 配信用（30MiB以下にビットレート上限）
execFileSync('ffmpeg', [
  '-y', '-v', 'error', '-i', O('philippines_ra9262_final.mp4'),
  '-c:v', 'libx264', '-preset', 'slow', '-crf', '21',
  '-maxrate', '3500k', '-bufsize', '7000k', '-pix_fmt', 'yuv420p',
  '-c:a', 'copy', '-movflags', '+faststart', O('philippines_ra9262_tiktok.mp4'),
], { stdio: 'inherit' });
console.log('done -> philippines_ra9262_tiktok.mp4');
